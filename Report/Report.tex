...
\begin{algorithm}
	...
	\For {$f_{i}=f_{1},f_{2},\ldots,f_{15},$} 
			...
			\For {$f_{n}= f_{n1},f_{n2},\ldots,f_{n10}$}
                ...
                \State Calculate the mean overshoot and setting time of 10 responses.
			\EndFor
		\EndFor
	\end{algorithmic} 
\end{algorithm}
...
Variables in this paper are: $K_{i}$, $K_{d}$, PID update frequency $f$, left wheel speed set point $v$, 
\begin{itemize}
    % 变量表
