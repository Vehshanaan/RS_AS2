...
\begin{itemize}
    ...
    \item \textbf{Trends in PID Parameters:}\\
    % 在我们为频率范围内的点调节PID参数时，我们观察到了以下特点：
    When we tuned the PID parameters for points in the frequency range, we observed the following characteristics:
    \begin{enumerate}
        ...
        \item \bm{$K_{d}$}: Starting at a outlier high value (1000.00) and decreasing with frequency, it suddenly increases at $f = 322.1$, and remains stable (i.e., in our test, $K_{d}$ has little influence to performance in high frequency band).
        \begin{figure}
            \centering
            \includegraphics[width=\linewidth]{Report/Pic/KiKdTrend.png}
