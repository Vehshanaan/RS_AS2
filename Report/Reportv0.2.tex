...
% 整个实验除PID参数为预先调节并储存好以外，均为自动进行。最大程度减少了人为干预。
 \textbf{The entire experiment is carried out automatically,} except for the PID parameters, which are pre-adjusted and stored. \textbf{Human intervention is minimised. The code we use has been shared on GitHub.} Detail of experiment see Algorithm \ref{TestMethod} and GitHub codes. 
...
\caption{Overall result picture. Red dots represents oscillate. Line label is center frequency of the neighborhood.  Detailed explanation see \ref{Result Details}.}% 遍历频率域的方法
\label{fig_appendix}
\end{figure*}
