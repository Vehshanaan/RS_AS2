 ...
 * @Date: 2022-11-11 11:00:11
 * @LastEditors: Runze Yuan 1959180242@qq.com
 * @LastEditTime: 2022-11-11 11:17:33
 * @FilePath: \RS_AS2\Tools\KinematicsExample\Encoder.h
 * @Description: 编码器头文件
 * 使用方法：
 * 1.setup()中调用setupEncoder0(); setupEncoder1()
 * 2.count_l count_r 即为左右轮编码器读数（long型），实时更新，调用即可
 * 备注：
 * 1.向前是正向后是负
 * 2.轮子转一圈是介于358-360之间的一个小数，暂取358.3
 * 3.轮子直径~32mm(各车可能不同)
 * 4.两轮距离~88.8mm(各车可能不同)
 * 5.在3.4.的基础上：一个编码代表：0.299675920792mm,以及1.004744627407201°
...
volatile long count_r; // 右轮 // used by encoder to count the rotation
volatile byte state_r;
volatile long count_l; // 左轮
volatile byte state_l;

#define code2dist 0.299675920792 // 编码乘以这个可以得到距离(单位：mm/读数)
#define pi 3.14159265            // π
#define code2angle 1.0047446     // 编码乘以这个可以得到轮子旋转的角度(单位：°/读数)
#define rad2deg 180 / 3.14159265 // 弧度转角度的方便参数
#define deg2rad 3.14159262 / 180 // 角度转弧度的方便参数
#define l 88.8                   // 两轮之间的距离约为88.8mm
...
ISR(INT6_vect)
{
    ...
    if (state_r == 0 || state_r == 3 || state_r == 5 || state_r == 6 || state_r == 8 || state_r == 10 || state_r == 12 || state_r == 15)
    { // 0,3,5,6,8,10,12,15: 要么无变化，要么一次变了两个（无效）
    }
    else if (state_r == 1 || state_r == 7 || state_r == 8 || state_r == 14)
    { // 1,7,8,14: +1
        // count_r+=1
        count_r -= 1;
    }
    else if (state_r == 2 || state_r == 4 || state_r == 11 || state_r == 13)
    { // 2,4,11,13: -1
        // count_r -=1
        count_r += 1;
...
ISR(PCINT0_vect)
{
    ...
    // Complete this if statement as necessary.
    // Refer to the labsheet.
    if (state_l == 0 || state_l == 3 || state_l == 5 || state_l == 6 || state_l == 8 || state_l == 10 || state_l == 12 || state_l == 15)
    { // 0,3,5,6,8,10,12,15: 要么无变化，要么一次变了两个（无效）
    }
    else if (state_l == 1 || state_l == 7 || state_l == 8 || state_l == 14)
    { // 1,7,8,14: +1
        // count_r+=1
        count_l -= 1;
    }
    else if (state_l == 2 || state_l == 4 || state_l == 11 || state_l == 13)
    { // 2,4,11,13: -1
        // count_r -=1
        count_l += 1;
    } // Continue this if statement as necessary.
...
void setupEncoder0()
{
    ...
    EIMSK = EIMSK & ~(1 << INT6); // 进行一系列处理前，先关闭INT6，这一条语句就是设EIMSK第六位为0的意思
    ...
    EICRB |= (1 << ISC60); // using header file names, push 1 to bit ISC60 // 开EIRCB的第六位
    ...
    EIFR |= (1 << INTF6); // 清除EIFR的第六位（复位，以防之前已设为有中断，造成误响应）
    ...
    EIMSK |= (1 << INT6); // 开EIMSK的INT6，使能中断输入
