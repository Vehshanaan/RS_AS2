 ...
 * @LastEditTime: 2022-11-10 17:30:58
 * @FilePath: \RS_AS2\Tools\PID\PID.h
 * @Description:PID类，使用方法如下：
 * 1.建立PID类的对象
 * 2.使用PID.init()来给三个参数以及积分限幅上限赋值
 * 3.计算PID结果：调用PID.cal(),括号里面输入被控量的当前值就可以，结果是PID.result,可以直接调用，因为是public
 * 4.如果想要重置PID，就用PID.reset(),他会在保持三参数与限幅值不变的同时，重置其他的一切
...
public:
     ...
     */
    void init(double Kp_, double Ki_, double Kd_, double I_limit_)
    {
        Kp = Kp_;
        Ki = Ki_;
        Kd = Kd_;
